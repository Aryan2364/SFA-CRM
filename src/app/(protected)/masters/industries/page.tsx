...
const COLS: Column[] = [
  { key: 'name', label: 'Industry / Segment' },
]

export default function IndustriesPage() {
  const crud = useCrud('/api/masters/industries')
  const me = useMe()
  ...
  const canEdit   = me?.permissions?.industries?.edit ?? false
  const canDelete = me?.permissions?.industries?.delete ?? false
      ...
      */}
      <Breadcrumb className="mb-4">
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage>Industry / Segment</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>

      <CrudPage
        title="Industry / Segment" columns={COLS}
        ...
        /* Delete is a soft delete. The toggle is how a deactivated segment comes
      ...
      />
      <Modal title={editing ? 'Edit Industry / Segment' : 'Add Industry / Segment'} isOpen={open} onClose={() => setOpen(false)} onSave={handleSave} isSaving={saving}>
        <div>
          <label htmlFor="industry-name" className="block text-sm font-medium text-text-secondary mb-1">Industry / Segment <span className="text-danger">*</span></label>
          <input id="industry-name" name="name" type="text" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Agriculture, Textiles & Apparel…"

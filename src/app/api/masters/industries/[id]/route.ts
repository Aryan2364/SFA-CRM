...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!await checkPermission(user, 'industries', 'edit')) return forbidden()
  const { name, sort_order, is_active } = await req.json()
  if (!name?.trim()) return NextResponse.json({ error: 'Name is required' }, { status: 400 })
  try {
    const data = await prisma.industries.update({
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  const user = await requireUser()
  if (!await checkPermission(user, 'industries', 'delete')) return forbidden()
  try {
    ...
    // `business_partners.industry_id` has no cascade, so a hard delete would
    // strand the FK; deactivating keeps existing companies readable.
    await prisma.industries.updateMany({
      where: { id: params.id, tenant_id: getTenantId() },

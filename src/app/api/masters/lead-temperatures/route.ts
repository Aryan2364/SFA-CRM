...
export async function GET() {
  ...
  await requireUser()
  try {
    const data = await prisma.lead_temperatures.findMany({
      where: {
        tenant_id: getTenantId(),
      },
      orderBy: { sort_order: 'asc' },
    })
    return NextResponse.json(serialize(data, 'lead_temperatures'))
  } catch (err) {
...
export async function POST(req: NextRequest) {
  ...
  try {
    const data = await prisma.lead_temperatures.create({
      data: { tenant_id: getTenantId(), name: name.trim(), sort_order: sort_order ?? 0 },
    })
    return NextResponse.json(serialize(data, 'lead_temperatures'), { status: 201 })
  } catch (err) {
    return NextResponse.json({ error: dbErrorMessage(err) }, { status: 500 })
  }
}

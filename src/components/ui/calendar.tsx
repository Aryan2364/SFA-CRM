...
function Calendar({
  ...
  // Section 20.1: the week starts on Monday.
  weekStartsOn = 1,
  locale,
  formatters,
  components,
  ...
  return (
    <DayPicker
      ...
      className={cn(
        "group/calendar bg-surface p-3 [--cell-radius:var(--radius-md)] [--cell-size:--spacing(7)] in-data-[slot=card-content]:bg-transparent in-data-[slot=popover-content]:bg-transparent",
      ...
      classNames={{
        ...
        weekday: cn(
          "flex-1 rounded-(--cell-radius) text-meta font-normal text-text-secondary select-none",
        ...
        week_number: cn(
          "text-meta text-text-muted select-none",
        ...
        today: cn(
          // Section 20.1: today is outlined, not filled. A filled today
          // reads as a selected day, which is exactly the confusion the
          // rule exists to prevent.
          "rounded-(--cell-radius) text-text-primary [&>button]:border [&>button]:border-primary data-[selected=true]:rounded-none",
...
function CalendarDayButton({
  ...
  return (
    <button
      ...
      className={cn(
        // A day cell is not a Button. Section 6.1 rule 4 makes every
        // Button carry a fill and a border, which turns a month grid
        // into a solid grey block. Section 20.1 gives the day its own
        // states instead: transparent at rest, today outlined, the
        ...
        "disabled:cursor-default disabled:text-text-muted disabled:hover:bg-transparent",
        "[&>span]:text-meta [&>span]:text-text-muted",
        defaultClassNames.day,
        className
      )}
      {...props}

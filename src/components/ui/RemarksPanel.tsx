...
export default function RemarksPanel({ isOpen, onClose, contextType, contextId, contextTitle }: RemarksPanelProps) {
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div className="fixed right-0 top-0 h-screen w-full sm:w-96 bg-surface shadow-2xl border-l border-border-light z-50 flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-4 border-b border-border-light shrink-0">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-xs text-text-muted uppercase tracking-wide font-normal">Remarks</p>
              <p className="text-sm font-medium text-text-primary truncate">{contextTitle}</p>
            </div>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-surface-control text-text-muted hover:text-text-secondary transition shrink-0">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          {loading ? (
            <div className="text-center py-12 text-text-muted text-sm">Loading remarks...</div>
          ) : topLevel.length === 0 ? (
            <div className="text-center py-12">
              <div className="w-12 h-12 bg-surface-control rounded-2xl flex items-center justify-center mx-auto mb-3">
            ...
            topLevel.map(remark => (
              <div key={remark.id}>
                ...
                {(repliesMap[remark.id] ?? []).length > 0 && (
                  <div className="ml-8 mt-2 space-y-2 border-l-2 border-border-light pl-3">
        ...
        {/* Reply indicator */}
        {replyTo && (
          <div className="px-4 py-2 bg-primary-subtle border-t border-primary-border flex items-center justify-between shrink-0">
            <div className="min-w-0">
              <p className="text-xs text-primary font-medium">Replying to {replyTo.users?.name ?? 'Unknown'}</p>
              <p className="text-xs text-text-muted truncate">{replyTo.body}</p>
            </div>
            <button onClick={() => setReplyTo(null)} className="shrink-0 ml-2 text-text-muted hover:text-text-secondary">
        ...
        {/* Compose */}
        <div className="px-4 py-3 border-t border-border-light bg-surface shrink-0">
          <div className="flex items-end gap-2">
            <textarea
              ...
              rows={2}
              className="flex-1 border border-border-light rounded-xl px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-primary-ring focus:border-transparent"
            />
            <button onClick={handleSend} disabled={sending || !body.trim()}
              className="w-10 h-10 bg-primary hover:bg-primary-hover disabled:opacity-40 text-primary-foreground rounded-xl flex items-center justify-center transition shrink-0">
    ...
    <div className={`flex items-start gap-2.5 ${isReply ? '' : ''}`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-baseline gap-2 mb-1">
          ...
          <span className="text-[10px] text-text-muted">{formatRelativeTime(remark.created_at)}</span>
          {!remark.is_read && (
            <span className="w-1.5 h-1.5 rounded-full bg-primary inline-block" />
          )}
        </div>
        <div className="bg-surface-sunken rounded-xl rounded-tl-none px-3 py-2">
          <p className="text-sm text-text-secondary whitespace-pre-wrap">{remark.body}</p>
        </div>
        {onReply && (
          <button onClick={onReply} className="mt-1 text-[10px] text-text-muted hover:text-primary transition">
            Reply
          </button>
        )}
      </div>
